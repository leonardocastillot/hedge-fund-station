...
from backend.hyperliquid_gateway.strategies.breakout_oi_confirmation.backtest import run_backtest
from backend.hyperliquid_gateway.strategies.breakout_oi_confirmation.logic import evaluate_signal
from backend.hyperliquid_gateway.strategies.breakout_oi_confirmation.paper import paper_candidate
from backend.hyperliquid_gateway.strategies.breakout_oi_confirmation.risk import build_risk_plan, calculate_position_size
from backend.hyperliquid_gateway.strategies.breakout_oi_confirmation.scoring import score_setup


class BreakoutOiConfirmationTest(unittest.TestCase):
    def test_evaluate_signal_long_short_and_no_trade(self) -> None:
        long_signal = evaluate_signal(base_market_data(change15m=0.32, change1h=0.70, change4h=0.80))
        self.assertEqual(long_signal["signal"], "long")

        short_signal = evaluate_signal(base_market_data(change15m=-0.32, change1h=-0.70, change4h=-0.80))
        self.assertEqual(short_signal["signal"], "short")

        no_trade = evaluate_signal(base_market_data(setupScores={"breakoutContinuation": 45, "fade": 20}))
        self.assertEqual(no_trade["signal"], "none")

    def test_score_setup_rewards_breakout_quality_and_liquidity(self) -> None:
        good_market = base_market_data(volume24h=350_000_000, openInterestUsd=300_000_000)
        weak_market = base_market_data(volume24h=1_000_000, openInterestUsd=500_000)
    ...
    def test_risk_blocks_cooldown_and_limits_concurrency(self) -> None:
        signal = {"signal": "long"}
        ...
        self.assertEqual(plan["stop_loss"], 100.55)
        self.assertEqual(plan["take_profit"], 98.95)

    def test_synthetic_backtest_generates_long_breakout_trade(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            db_path = Path(tmp) / "hyperliquid.db"
            create_market_db(db_path)
            base_ts = 1_000_000_000_000
            prices = [100.0] * 12 + [101.0, 103.0]
            for index, price in enumerate(prices, start=1):
                insert_snapshot(db_path, index, base_ts + ((index - 1) * 300_000), "BTC", price, open_interest_usd=2_000_000 + index * 20_000)
        ...
        self.assertEqual(trade["strategy_id"], "breakout_oi_confirmation")
        ...
        standby = paper_candidate(
            {
                ...
                "report_summary": {"total_trades": 20},
    ...
    def test_registry_exposes_strategy(self) -> None:
        self.assertIn("breakout_oi_confirmation", available_strategies())
...
def base_market_data(**overrides: object) -> dict[str, object]:
    data: dict[str, object] = {
        ...
        "openInterestUsd1hAgo": 250_000_000,
        "volume24h": 350_000_000,
        "opportunityScore": 74,
        "crowdingBias": "balanced",
        "primarySetup": "breakout-continuation",
        "setupScores": {"breakoutContinuation": 82, "fade": 20, "shortSqueeze": 55, "longFlush": 10},
    }
...
def insert_snapshot(db_path: Path, row_id: int, timestamp_ms: int, symbol: str, price: float, *, open_interest_usd: float) -> None:
    with sqlite3.connect(db_path) as connection:
        connection.execute(
            """
            ...
            VALUES (?, ?, ?, ?, 1.6, ?, 350000000.0, 0.00001, 74.0,
                    'watch', 'normal', 350000.0, 'balanced', 'breakout-continuation', ?)
            """,
            (
                row_id,
                timestamp_ms,
                ...
                price,
                open_interest_usd,
                json.dumps({"breakoutContinuation": 82, "fade": 20, "shortSqueeze": 55, "longFlush": 10}),
            ),
        )
