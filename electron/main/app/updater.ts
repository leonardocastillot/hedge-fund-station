...
export class UpdateManager {
  ...
  public checkForUpdates() {
    if (process.env.HEDGE_STATION_ENABLE_UPDATES !== '1') {
      log.info('Skipping update check because local hedge build updates are disabled');
      return;
    }
    ...
    if (!app.isPackaged || !existsSync(updateConfigPath)) {
      log.info('Skipping update check because packaged update config is not available');
      return;
    }
