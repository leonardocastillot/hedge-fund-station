...
export default function StrategyLibraryPage() {
  ...
  const summary = useMemo(() => {
    ...
    return {
      ...
      deployed,
      avgExecution,
      legacyStrategies: strategies.filter((strategy) => strategy.source === 'legacy').length
    };
  }, [strategies]);
    ...
    <div className="mx-auto flex max-w-7xl flex-col gap-4 px-4 py-6">
      <div className="rounded-[24px] border border-cyan-500/15 bg-[linear-gradient(135deg,rgba(8,145,178,0.18),rgba(15,23,42,0.9))] p-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-2 max-w-3xl text-sm text-slate-300">
              When the legacy trading backend is available, this page uses its strategy cache, backtests and deployment state. If not, it falls back to the Hyperliquid gateway.
            </p>
          </div>
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-3">
          ...
          <SummaryCard label="Deployed" value={String(summary.deployed)} detail={`${summary.legacyStrategies} from legacy stack`} />
