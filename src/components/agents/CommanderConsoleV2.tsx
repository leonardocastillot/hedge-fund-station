...
export const CommanderConsoleV2: React.FC<{ workspaceId?: string | null }> = ({ workspaceId }) => {
  ...
  const latestRunningRun = React.useMemo(
    () => scopedRuns.find((run) => run.status === 'running'),
    [scopedRuns]
  );
  ...
  const {
    ...
    onTranscript: (text) => setGoal(text)
  });
      ...
      <div style={heroGridStyle}>
        <div style={composerCardStyle}>
          ...
          <div style={voiceDockStyle}>
            ...
            >
              {voiceStatus === 'recording' ? `Listening ${durationSeconds}s` : 'Hold To Speak'}
            </button>
            <div style={{ minWidth: 0, flex: 1 }}>
              <div style={{ color: voiceError ? '#fca5a5' : '#94a3b8', fontSize: '11px' }}>
                {voiceError || (voiceStatus === 'transcribing' ? 'Transcribing voice mission...' : 'Voice fills the mission box')}
              </div>
              {transcript ? (
