...
import { CommanderConsoleV2 } from '../agents/CommanderConsoleV2';
import { KnowledgeDock } from '../agents/KnowledgeDock';
import { AgentVisualizer } from '../agents/AgentVisualizer';
import { SystemHealthCard } from '../agents/SystemHealthCard';
import { LaunchSignalStrip } from '../agents/LaunchSignalStrip';
...
type PanelView = 'launch' | 'fleet' | 'runs' | 'intel';
...
export const AgentsPanel: React.FC = () => {
  ...
  const [activeView, setActiveView] = React.useState<PanelView>('launch');
  ...
  const handleAgentClick = React.useCallback((agentId: string) => {
    ...
    const agentTerminals = terminals.filter((t) => t.agentId === agentId);
    if (agentTerminals.length > 0) {
      setActiveTerminal(agentTerminals[0].id);
    }
  ...
  return (
    <div style={pageStyle}>
      <div style={heroStyle}>
        <div>
          <div style={heroEyebrowStyle}>Agentic Ops</div>
          <h2 style={heroTitleStyle}>AI Station</h2>
          <p style={heroCopyStyle}>Compact mission control for launch, fleet, runs and knowledge.</p>
        </div>
      ...
      <div style={bodyStyle}>
        ...
        {activeView === 'launch' ? (
          <>
            ...
            <CommanderConsoleV2 workspaceId={workspaceId} />
          </>
        ) : null}
