"""Backtest adapter for BTC Z-score ATR Trend."""
...
def run_backtest(dataset_path: Path, config: BacktestConfig) -> dict[str, Any]:
    ...
    if not rows:
        raise ValueError("BTC Z-score ATR Trend requires non-empty BTC/USD daily history.")
    ...
    diagnostics = build_trade_diagnostics(
        ...
        robust_gate=ZSCORE_ROBUST_GATE,
    )
    ...
    return {
        ...
        "notes": [
            "BTC daily Z-score ATR trend replay using shared BTC/USD history.",
            "Long-only with Z-score bounded entry (-0.5 to 1.5) and Z-score exit (-1.2).",
            "Position sizing via risk-budgeting (target_risk / ATR_stop).",
...
def build_latest_signal(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        **signal_eval,
        ...
        "trigger_plan": "Enter long when close > SMA200, SMA50 > SMA200, RSI14 > 40, ATR percentile < 82, "
                        "Z-score(close,50) between -0.5 and 1.5. Size via risk-budgeting.",
        "invalidation_plan": "Exit on Z-score < -1.2, ATR trailing stop (4.5x ATR from peak), "
