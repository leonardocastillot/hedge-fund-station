...
def run_backtest(dataset_path: Path, config: BacktestConfig) -> dict[str, Any]:
    ...
    for row in sampled_rows:
        ...
        if open_position is not None and open_position["symbol"] == symbol:
            ...
            if maybe_trade is not None:
                ...
                cooldown_ms = THIRTY_MINUTES_MS if float(maybe_trade["net_pnl"]) < 0 else TEN_MINUTES_MS
                cooldown_until_by_symbol[symbol] = int(market_data["timestamp_ms"]) + cooldown_ms
                open_position = None

        equity_curve.append({"timestamp": market_data["timestamp"], "equity": round(equity, 2)})
        if len(history) < 4 or open_position is not None:
            continue
        ...
        open_position = {
            ...
            "stop_loss": round(entry_price * 0.9975, 6),
            "take_profit": round(entry_price * 1.0035, 6),
            "entry_context": {
                "signal": signal_eval,
                "score": setup_score,
                "risk_plan": risk_plan,
    ...
    if open_position is not None:
        ...
        trade = close_position(open_position, latest_market_data["timestamp"], float(latest_market_data["price"]), "forced_close", config)
    ...
    diagnostics = build_trade_diagnostics(
        ...
        robust_gate=SCALPER_ROBUST_GATE,
    )
    return {
        "dataset": {
        ...
        "notes": [
            "BTC-first taker scalper replay on Hyperliquid gateway market snapshots.",
            "Default universe is BTC unless symbols or universe=all is explicitly provided.",
            "Passing the gate only enables paper review; this is not a live-trading route.",
        ],
    }
...
def maybe_close_position(position: dict[str, Any], market_data: dict[str, Any], config: BacktestConfig) -> dict[str, Any] | None:
    ...
    if held_ms >= TEN_MINUTES_MS and abs(pnl_pct) < 0.08:
        return close_position(position, market_data["timestamp"], apply_slippage(price, "long", int(market_data.get("executionQuality", 60)), True), "no_progress", config)
    if held_ms >= TWENTY_MINUTES_MS:
    ...
    if held_ms >= FIVE_MINUTES_MS and float(market_data.get("change15m", 0.0) or 0.0) <= -0.20:
        return close_position(position, market_data["timestamp"], apply_slippage(price, "long", int(market_data.get("executionQuality", 60)), True), "micro_reversal", config)
    return None
...
def build_latest_signal(symbol_histories: dict[str, list[dict[str, Any]]]) -> dict[str, Any]:
    ...
    if not candidates:
        return {"strategy_id": "btc_crowding_scalper", "signal": "none"}
    ...
    return {
        **signal_eval,
        ...
        "trigger_plan": "Enter long when crowding tailwind, micro impulse, OI stability, and liquid execution align.",
        "invalidation_plan": "Exit on -0.25% stop, +0.35% target, no-progress after 10m, 20m time stop, or micro reversal.",
    }
...
def apply_slippage(price: float, side: str, execution_quality: int, is_exit: bool) -> float:
    base = 0.00025 if execution_quality >= 75 else 0.00045 if execution_quality >= 55 else 0.00075
