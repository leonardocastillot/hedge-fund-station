...
def run_backtest(dataset_path: Path, config: BacktestConfig) -> dict[str, Any]:
    ...
    if not rows:
        raise ValueError("BTC Multi-Frame Ensemble requires non-empty BTC/USD daily history.")
    ...
    return {
        ...
        "notes": [
            "Multi-timeframe trend ensemble. 4 MA pairs vote independently.",
            "Smooth vol-scaled ATR trailing (2.5x-6.0x continuous, no step functions).",
            "Structure exit on trend consensus breakdown (score <= 1).",
            "Conviction-based position sizing from trend score + vol percentile.",
        ],
    }
...
def build_latest_signal(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        **signal_eval,
        ...
        "trigger_plan": "Enter long when trend_score >= 3, RSI 40-75, ATR pct < 85, "
                        "and pullback (price between MA20/MA50, RSI < 58) or momentum "
                        "(price > MA20, MA20 slope > 0.1%, RSI >= 48) confirmed.",
        "invalidation_plan": "Exit on smooth ATR trailing stop (2.5-6.0x continuous, vol-scaled), "
