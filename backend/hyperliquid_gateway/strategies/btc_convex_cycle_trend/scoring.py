"""Setup scoring for BTC Convex Cycle Trend."""
...
def score_setup(signal_eval: dict[str, Any]) -> dict[str, Any]:
    ...
    momentum_30d_pct = float(signal_eval.get("momentum_30d_pct") or 0.0)
    rsi_edge = max(0.0, rsi14 - MIN_ENTRY_RSI)
    drawdown_penalty = min(35.0, float(signal_eval.get("drawdown_180d_pct") or 0.0) * 0.45)
    convex_bonus = 14.0 if signal_eval.get("convex_regime") else 0.0
    exposure_bonus = float(signal_eval.get("target_exposure_fraction") or 0.0) * 45.0
    rank_score = max(
        0.0,
        min(
            100.0,
            30.0
            + trend_spread_pct
            + (price_spread_pct * 0.35)
            + (momentum_30d_pct * 0.25)
            + (rsi_edge * 0.75)
            - drawdown_penalty
            + convex_bonus
            + exposure_bonus,
    ...
    return {
        ...
        "convex_regime": bool(signal_eval.get("convex_regime")),
