...
TIGHT_TRAIL_MULT = 3.5
WIDE_TRAIL_MULT = 5.5
TRAIL_TRANSITION_DAY = 15
...
def evaluate_signal(
    ...
    *,
    ...
    result["reasons"] = build_reasons(result, entry_trigger=entry_trigger, exit_reason=exit_reason, vol_regime=vol_regime, days_in=days_in)
    return result


def atr_stop_pct(atr: float | None, close: float, multiplier: float) -> float:
...
def build_reasons(
    ...
    *,
    ...
    phase = "tight" if days_in < TRAIL_TRANSITION_DAY else "wide"
    if exit_reason == "atr_trailing_stop":
        return [f"Close is {context['trade_drawdown_pct']:.2f}% below trade peak (progressive {phase} trail: {context.get('dynamic_multiplier', 0):.1f}x)."]
    ...
    if entry_trigger:
        return [
            ...
            f"Progressive trail: {TIGHT_TRAIL_MULT}x (first {TRAIL_TRANSITION_DAY}d) → {WIDE_TRAIL_MULT}x (after). "
            f"target_size={context.get('target_position_size_pct', 0):.2f}%."
        ]
    if vol_regime == "extreme":
        return [f"Vol regime extreme (ATR percentile {context['atr_percentile']:.0f}%). No entry."]
    return [f"No entry trigger. Vol regime: {vol_regime} (day {days_in}, {phase} trail)."]
...
def row_close(row: dict[str, Any]) -> float:
    ...
    if value is None:
        raise ValueError("BTC regime adaptive rows require close or price.")
    return float(value)
