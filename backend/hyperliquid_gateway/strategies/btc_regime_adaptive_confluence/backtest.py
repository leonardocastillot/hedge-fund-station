...
def run_backtest(dataset_path: Path, config: BacktestConfig) -> dict[str, Any]:
    ...
    if not rows:
        raise ValueError("BTC Regime Adaptive Confluence requires non-empty BTC/USD daily history.")
    ...
    return {
        ...
        "notes": [
            "BTC daily progressive trend using champion-like SMA50/SMA150 entry.",
            "Progressive trailing: 3.5x ATR first 15 days (tight, cut losers early), then 5.5x ATR (wide, let winners run).",
            "Same risk-budgeting sizing as champion.",
        ],
    }
...
def build_latest_signal(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        **signal_eval,
        ...
        "trigger_plan": "Enter long when close > SMA150, SMA50 > SMA150, RSI14 > 42, ATR percentile < 82, "
                        "and pullback or momentum condition met.",
        "invalidation_plan": "Exit on progressive ATR trailing stop (3.5x first 15d, then 5.5x), "
