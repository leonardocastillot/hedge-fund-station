"""Backtest adapter for BTC Vol Tight Trail Trend."""
...
def run_backtest(dataset_path: Path, config: BacktestConfig) -> dict[str, Any]:
    ...
    if not rows:
        raise ValueError("BTC Vol Tight Trail Trend requires non-empty BTC/USD daily history.")
    ...
    diagnostics = build_trade_diagnostics(
        ...
        robust_gate=TIGHT_TRAIL_ROBUST_GATE,
    )
    ...
    return {
        ...
        "notes": [
            "BTC daily volatility-regime tight trail trend replay using shared BTC/USD history.",
            "Long-only with risk-budgeting via ATR; no shorts, no leverage, no live routing.",
            "Position size = equity * target_risk_pct / (3.5*ATR/close).",
...
def build_latest_signal(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        **signal_eval,
        ...
        "invalidation_plan": "Exit on ATR trailing stop (3.5x ATR from peak), trend break (close < SMA150), or time stop (200 days).",
    }
