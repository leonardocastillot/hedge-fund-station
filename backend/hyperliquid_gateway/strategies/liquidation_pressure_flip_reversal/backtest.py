...
def run_backtest(dataset_path: Path, config: BacktestConfig) -> dict[str, Any]:
        ...
        if symbol in open_positions:
            ...
            if maybe_trade is not None:
                ...
                cooldown_until_by_symbol[symbol] = int(market_data["timestamp_ms"]) + (
                    35 * 60 * 1000 if float(maybe_trade["net_pnl"]) < 0 else FIVE_MINUTES_MS
                )
    ...
    diagnostics = build_trade_diagnostics(
        ...
        robust_gate=LIQUIDATION_FLIP_ROBUST_GATE,
    )
    return {
        "dataset": {
        ...
        "notes": [
            "Gateway snapshot replay for Liquidation Pressure Flip Reversal.",
            "Uses persisted liquidation estimate, crowding bias, setup scores, price displacement, OI, funding, and liquidity.",
...
def maybe_close_position(position: dict[str, Any], market_data: dict[str, Any], config: BacktestConfig) -> dict[str, Any] | None:
    ...
    if held_ms >= FIFTEEN_MINUTES_MS:
        ...
        if side == "short" and float(market_data.get("change15m", 0.0) or 0.0) >= 0.28:
            return close_position(position, market_data["timestamp"], apply_slippage(price, side, execution_quality, True), "impulse_reasserted", config)
        if oi_delta_from_entry >= 2.5 and pnl_pct < 0.05:
            return close_position(position, market_data["timestamp"], apply_slippage(price, side, execution_quality, True), "oi_impulse_reloaded", config)
    if held_ms >= TWENTY_MINUTES_MS and abs(pnl_pct) < 0.10:
...
def build_latest_signal(symbol_histories: dict[str, list[dict[str, Any]]]) -> dict[str, Any]:
    ...
    return {
        **signal_eval,
        ...
        "trigger_plan": "Enter only after visible liquidation pressure, stretched crowding, and 5m/15m stall align.",
        "invalidation_plan": "Exit on dynamic stop, impulse reassertion, OI reload, no-progress after 20m, or 60m time stop.",
    }
