...
from .logic import STRATEGY_ID, evaluate_signal
...
TWENTY_FIVE_MINUTES_MS = 25 * 60 * 1000
ONE_HOUR_MS = 60 * 60 * 1000
FOUR_HOURS_MS = 4 * ONE_HOUR_MS
TWO_HOURS_MS = 120 * 60 * 1000
SEVEN_DAYS_MS = 7 * 24 * ONE_HOUR_MS
DEFAULT_SYMBOLS = ("BTC", "SOL", "HYPE", "ETH", "DOGE", "AAVE", "LINK", "SUI", "APT", "ARB")

STRUCTURAL_ROBUST_GATE = {
    "min_trades": 20,
    "min_return_pct": 0.25,
    "min_profit_factor": 1.2,
    "max_drawdown_pct": 8.0,
    "min_avg_net_trade_return_pct": 0.05,
...
def run_backtest(dataset_path: Path, config: BacktestConfig) -> dict[str, Any]:
    ...
    for row in sampled_rows:
        ...
        if symbol in open_positions:
            ...
            if maybe_trade is not None:
                ...
                cooldown_until_by_symbol[symbol] = int(market_data["timestamp_ms"]) + (
                    FIFTEEN_MINUTES_MS if float(maybe_trade["net_pnl"]) < 0 else FIVE_MINUTES_MS
                )
        ...
        risk_plan = build_risk_plan({**market_data, "price": entry_price, "side": side, "conviction": signal_eval.get("conviction", 50), "executionQuality": setup_score["execution_quality"]}, side=side)
    ...
    diagnostics = build_trade_diagnostics(
        ...
        robust_gate=STRUCTURAL_ROBUST_GATE,
    )
    return {
        "dataset": {
        ...
        "notes": [
            "Hyperliquid Structural Alpha — multi-factor composite exploiting funding extremes, "
            "OI divergence, setup confluence, and multi-TF momentum.",
            "Long/short across 10+ symbols. 5-min snapshot replay.",
            "Passing validation only permits paper review; not a live-trading route.",
        ],
    }
...
def build_market_data(history: list[dict[str, Any]], row: dict[str, Any]) -> dict[str, Any]:
    ...
    funding_history = [item.get("fundingRate") or item.get("funding_rate") or 0.0 for item in history if item["timestamp_ms"] >= row["timestamp_ms"] - SEVEN_DAYS_MS]
...
def maybe_close_position(position: dict[str, Any], market_data: dict[str, Any], config: BacktestConfig) -> dict[str, Any] | None:
    ...
    eq = int(market_data.get("executionQuality", 60) or 60)

    if side == "long":
        if price <= float(position["stop_loss"]):
            return close_position(position, market_data["timestamp"], apply_slippage(price, side, eq, True), "stop_loss", config)
        if price >= float(position["take_profit"]):
            return close_position(position, market_data["timestamp"], apply_slippage(price, side, eq, True), "take_profit", config)
    else:
        if price >= float(position["stop_loss"]):
            return close_position(position, market_data["timestamp"], apply_slippage(price, side, eq, True), "stop_loss", config)
        if price <= float(position["take_profit"]):
    ...
    oi_delta = ((current_oi - entry_oi) / entry_oi) * 100.0 if entry_oi else 0.0

    if oi_delta <= -2.5 and held_ms >= FIFTEEN_MINUTES_MS:
        return close_position(position, market_data["timestamp"], apply_slippage(price, side, eq, True), "oi_contraction", config)
    if held_ms >= FIFTEEN_MINUTES_MS:
        if side == "long" and float(market_data.get("change15m", 0.0) or 0.0) <= -0.25:
            return close_position(position, market_data["timestamp"], apply_slippage(price, side, eq, True), "direction_reversed", config)
        if side == "short" and float(market_data.get("change15m", 0.0) or 0.0) >= 0.25:
            return close_position(position, market_data["timestamp"], apply_slippage(price, side, eq, True), "direction_reversed", config)
    if held_ms >= TWENTY_FIVE_MINUTES_MS and abs(pnl_pct) < 0.10:
        return close_position(position, market_data["timestamp"], apply_slippage(price, side, eq, True), "no_progress", config)
    if held_ms >= TWO_HOURS_MS:
        return close_position(position, market_data["timestamp"], apply_slippage(price, side, eq, True), "time_stop", config)
    return None
...
def build_latest_signal(symbol_histories: dict[str, list[dict[str, Any]]]) -> dict[str, Any]:
    ...
    return {
        **signal_eval,
        ...
        "trigger_plan": "Composite score crosses +/-50 with >= 2 timeframes agreeing.",
        "invalidation_plan": "Stop loss, take profit, OI contraction, 15m reversal, 25m no-progress, 120m time stop.",
    }
...
def apply_slippage(price: float, side: str, execution_quality: int, is_exit: bool) -> float:
    base = 0.00035 if execution_quality >= 75 else 0.0006 if execution_quality >= 55 else 0.0010
...
def compact_market_context(market_data: dict[str, Any]) -> dict[str, Any]:
    keys = [
        "timestamp", "symbol", "price", "fundingPercentile",
        "change5m", "change15m", "change1h", "change4h",
        "openInterestUsd", "openInterestUsd1hAgo", "volume24h",
        "crowdingBias", "primarySetup", "setupScores", "executionQuality",
    ]
