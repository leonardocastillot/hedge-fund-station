...
def run_backtest(dataset_path: Path, config: BacktestConfig) -> dict[str, Any]:
    ...
    if not rows:
        raise ValueError("BTC Trend Cascade requires non-empty BTC/USD daily history.")
    ...
    if open_position is not None:
        ...
        trade = close_position(open_position, latest,
                               {**signal_eval, "exit_reason": "forced_close"}, config)
        cash += float(trade["exit_cash_added"])
        fees_paid += float(trade["exit_fee"])
        trades.append(trade)
    ...
    return {
        ...
        "notes": [
            "BTC Trend Cascade: multi-timeframe momentum cascade with conviction scoring.",
            "No SMA50/SMA150 crossovers — uses ROC5/10/20/50 momentum on ADX trend filter.",
            "Exit on momentum collapse (immediate), progressive ATR trail, or trend fade.",
            "Conviction-based sizing: stronger momentum = larger position.",
        ],
    }


def close_position(position: dict[str, Any], row: dict[str, Any],
                   signal_eval: dict[str, Any], config: BacktestConfig) -> dict[str, Any]:
...
def build_latest_signal(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        **signal_eval,
        ...
        "trigger_plan": "Enter when cascade momentum score >= 0.5, ADX > 22, vol < 85%ile.",
        "invalidation_plan": "Exit on momentum collapse (score < -0.1), progressive ATR trail "
                            "(2.5x first 12d, 5.0x after), ADX < 16, or time stop (150 days).",
    }
