"""Backtest adapter for BTC Adaptive Cycle Trend."""
...
PAPER_READY_BENCHMARK_RETURN_PCT = 89.53
ADAPTIVE_CYCLE_ROBUST_GATE = {
    "min_trades": 10,
    "min_return_pct": PAPER_READY_BENCHMARK_RETURN_PCT,
...
def run_backtest(dataset_path: Path, config: BacktestConfig) -> dict[str, Any]:
    ...
    if not rows:
        raise ValueError("BTC Adaptive Cycle Trend requires non-empty BTC/USD daily history.")
    ...
    summary = {
        **summary,
        "strategy_id": STRATEGY_ID,
        "paper_ready_benchmark_strategy": "btc_guarded_cycle_trend",
        "paper_ready_benchmark_return_pct": PAPER_READY_BENCHMARK_RETURN_PCT,
        "excess_return_vs_paper_ready_benchmark_pct": round(float(summary.get("return_pct", 0.0)) - PAPER_READY_BENCHMARK_RETURN_PCT, 2),
        "beats_paper_ready_benchmark": float(summary.get("return_pct", 0.0)) > PAPER_READY_BENCHMARK_RETURN_PCT,
    }
    diagnostics = build_trade_diagnostics(
        ...
        robust_gate=ADAPTIVE_CYCLE_ROBUST_GATE,
    )
    ...
    return {
        ...
        "benchmark": {
            ...
            "return_pct": PAPER_READY_BENCHMARK_RETURN_PCT,
            "initial_equity": 500.0,
            "fee_model": "taker",
            "leverage": "none",
        },
        "notes": [
            "BTC daily adaptive cycle trend replay using shared BTC/USD history.",
            "Long-only adaptive partial exposure; no shorts, no leverage, no live routing.",
            "Validation requires beating the 89.53% paper-ready BTC guarded cycle benchmark before paper review.",
        ],
    }
