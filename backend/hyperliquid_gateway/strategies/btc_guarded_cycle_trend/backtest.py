"""Backtest adapter for BTC Guarded Cycle Trend."""
...
def run_backtest(dataset_path: Path, config: BacktestConfig) -> dict[str, Any]:
    ...
    if not rows:
        raise ValueError("BTC Guarded Cycle Trend requires non-empty BTC/USD daily history.")
    ...
    for index, row in enumerate(rows):
        ...
        if open_position is None:
            ...
            if signal_eval.get("signal") == "long":
                sizing = calculate_position_size(portfolio_value=cash, risk_fraction=config.risk_fraction)
                if sizing["can_enter"]:
    ...
    summary = {**summary, "strategy_id": STRATEGY_ID}
    diagnostics = build_trade_diagnostics(
        summary=summary,
        trades=trades,
        initial_equity=config.initial_equity,
        requested_symbols=[SYMBOL],
        robust_gate=GUARDED_CYCLE_ROBUST_GATE,
    )
    ...
    return {
        ...
        "notes": [
            "BTC daily guarded cycle trend replay using shared BTC/USD history.",
            "Long-only partial exposure; no shorts, no leverage, no live routing.",
            "Validation target requires at least 50% net return after fees before paper review.",
        ],
    }
