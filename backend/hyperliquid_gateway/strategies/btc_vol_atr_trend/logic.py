"""Deterministic BTC daily volatility-regime ATR trend logic."""
...
def evaluate_signal(
    ...
    *,
    ...
    trailing_exit = bool(
        in_position
        and ctx["trade_drawdown_pct"] >= atr_stop_pct(atr, close)
    )
    trend_exit = bool(
        in_position
        and close < sma200
    ...
    atr_stop_dist_pct = atr_stop_pct(atr, close)
...
def atr_stop_pct(atr: float, close: float) -> float:
    if close <= 0:
        return 0.0
    ...
    *,
    ...
    if entry_trigger:
        return [
            ...
            f"stop_dist={context.get('atr_stop_distance_pct', 0):.2f}%, "
    ...
    return [f"No ATR trend entry trigger. Vol regime: {vol_regime}."]
...
def row_close(row: dict[str, Any]) -> float:
    ...
    if value is None:
        raise ValueError("BTC ATR trend rows require close or price.")
    return float(value)
