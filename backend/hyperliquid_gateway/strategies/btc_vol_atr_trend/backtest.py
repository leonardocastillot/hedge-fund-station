"""Backtest adapter for BTC Vol ATR Trend."""
...
def run_backtest(dataset_path: Path, config: BacktestConfig) -> dict[str, Any]:
    ...
    if not rows:
        raise ValueError("BTC Vol ATR Trend requires non-empty BTC/USD daily history.")
    ...
    diagnostics = build_trade_diagnostics(
        ...
        robust_gate=VOL_ATR_ROBUST_GATE,
    )
    ...
    return {
        ...
        "notes": [
            "BTC daily volatility-regime ATR trend replay using shared BTC/USD history.",
            "Long-only with risk-budgeting via ATR; no shorts, no leverage, no live routing.",
            "Position size = equity * target_risk_pct / (3*ATR/close).",
            "Validation requires beating current 115.78% BTC convex cycle champion before paper review.",
        ],
    }
...
def build_latest_signal(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        **signal_eval,
        ...
        "trigger_plan": "Enter long when close > SMA200, SMA50 > SMA200, RSI14 > 38, ATR percentile < 85. "
                        "Position sizing via risk-budgeting: size = equity * target_risk_pct / (3*ATR/close*100).",
        "invalidation_plan": "Exit on ATR-based trailing stop (3x ATR from peak), trend break (close < SMA200), or time stop (240 days).",
    }
