"""Risk helpers for Breakout OI Confirmation."""
...
def build_risk_plan(context: dict[str, Any], side: str | None = None) -> dict[str, Any]:
    ...
    stop_loss_pct = 0.55 if execution_quality >= 75 else 0.70 if execution_quality >= 55 else 0.90
    take_profit_pct = 1.05 if execution_quality >= 75 else 1.25 if execution_quality >= 55 else 1.45
    ...
    return {
        ...
        "invalidation": [
            ...
            "Exit if OI contracts materially after entry.",
            "Exit if breakout direction reverses on the 15 minute window.",
            "Exit if the trade has no progress after 25 minutes.",
            "Exit after 90 minutes if target is not reached.",
            "Pause the symbol for 25 minutes after a loss.",
        ],
    }
...
def calculate_position_size(
    *,
    ...
    size_pct = 0.010 if execution_quality >= 75 else 0.007 if execution_quality >= 55 else 0.005
    if len(open_positions) >= 1:
        size_pct = min(size_pct, 0.007)
    if len(open_positions) >= 2:
        size_pct = min(size_pct, 0.005)
