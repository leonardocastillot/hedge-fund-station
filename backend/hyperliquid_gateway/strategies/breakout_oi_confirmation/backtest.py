...
BREAKOUT_ROBUST_GATE = {
    "min_trades": 20,
    "min_return_pct": 0.15,
    "min_profit_factor": 1.15,
    "max_drawdown_pct": 6.0,
    "min_avg_net_trade_return_pct": 0.05,
    "max_largest_trade_pnl_share_pct": 60.0,
    "max_exit_reason_pnl_share_pct": 88.0,
}
...
def run_backtest(dataset_path: Path, config: BacktestConfig) -> dict[str, Any]:
        ...
        if symbol in open_positions:
            ...
            if maybe_trade is not None:
                ...
                cooldown_until_by_symbol[symbol] = int(market_data["timestamp_ms"]) + (
                    25 * 60 * 1000 if float(maybe_trade["net_pnl"]) < 0 else FIVE_MINUTES_MS
                )
    ...
    diagnostics = build_trade_diagnostics(
        ...
        robust_gate=BREAKOUT_ROBUST_GATE,
    )
    return {
        "dataset": {
        ...
        "notes": [
            "Gateway snapshot replay for Breakout OI Confirmation.",
            "Uses price displacement, OI expansion, liquidity, funding context, crowding bias, and setup scores already persisted in market_snapshots.",
            "No raw orderbook or trade-aggression claim is made in v1.",
...
def maybe_close_position(position: dict[str, Any], market_data: dict[str, Any], config: BacktestConfig) -> dict[str, Any] | None:
    ...
    oi_delta_from_entry = ((current_oi - entry_oi) / entry_oi) * 100 if entry_oi else 0.0
    if oi_delta_from_entry <= -1.2 and held_ms >= FIFTEEN_MINUTES_MS:
        return close_position(position, market_data["timestamp"], apply_slippage(price, side, execution_quality, True), "oi_confirmation_lost", config)
    if held_ms >= FIFTEEN_MINUTES_MS:
        if side == "long" and float(market_data.get("change15m", 0.0) or 0.0) <= -0.20:
            return close_position(position, market_data["timestamp"], apply_slippage(price, side, execution_quality, True), "breakout_reversed", config)
        if side == "short" and float(market_data.get("change15m", 0.0) or 0.0) >= 0.20:
            return close_position(position, market_data["timestamp"], apply_slippage(price, side, execution_quality, True), "breakout_reversed", config)
...
def build_latest_signal(symbol_histories: dict[str, list[dict[str, Any]]]) -> dict[str, Any]:
    ...
    return {
        **signal_eval,
        ...
        "trigger_plan": "Enter only when breakout direction, OI expansion, liquidity, and breakoutContinuation score align.",
        "invalidation_plan": "Exit on dynamic stop, OI confirmation loss, 15m reversal, no-progress after 25m, or 90m time stop.",
    }
