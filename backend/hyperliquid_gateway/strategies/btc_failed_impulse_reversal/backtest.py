...
FAILED_IMPULSE_ROBUST_GATE = {
    "min_trades": 8,
    "min_return_pct": 0.50,
    "min_profit_factor": 1.50,
    "max_drawdown_pct": 4.0,
    "min_avg_net_trade_return_pct": 0.25,
    "max_largest_trade_pnl_share_pct": 35.0,
    "max_exit_reason_pnl_share_pct": 95.0,
}
...
def run_backtest(dataset_path: Path, config: BacktestConfig) -> dict[str, Any]:
    ...
    diagnostics = build_trade_diagnostics(
        ...
        robust_gate=FAILED_IMPULSE_ROBUST_GATE,
    )
    return {
        "dataset": {
        ...
        "notes": [
            "BTC-only failed impulse reversal replay on Hyperliquid gateway market snapshots.",
...
def apply_slippage(price: float, side: str, execution_quality: int, is_exit: bool) -> float:
    base = 0.0003 if execution_quality >= 75 else 0.00045 if execution_quality >= 55 else 0.00075
