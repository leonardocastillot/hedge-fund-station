"""Backtest adapter for BTC Asymmetric Vol Carry (long-only)."""
...
from .logic import STRATEGY_ID, SYMBOL, SIDE_LONG, evaluate_signal, row_close
from .risk import build_risk_plan, calculate_position_size
from .scoring import score_setup

CHAMPION_STRATEGY = "btc_convex_cycle_trend"
CHAMPION_RETURN_PCT = 116.0
ASYMMETRIC_VOL_ROBUST_GATE = {
    "min_trades": 15,
    "min_return_pct": CHAMPION_RETURN_PCT,
    "min_profit_factor": 1.8,
    "max_drawdown_pct": 22.0,
    "min_avg_net_trade_return_pct": 1.0,
...
def run_backtest(dataset_path: Path, config: BacktestConfig) -> dict[str, Any]:
    ...
    if not rows:
        raise ValueError("BTC Asymmetric Vol Carry requires non-empty BTC/USD daily history.")
    ...
    for index, row in enumerate(rows):
        ...
        if open_position is not None:
            peak = max(float(open_position["peak_close"]), close)
            open_position["peak_close"] = peak

            exit_eval = evaluate_signal(
                rows, index,
                in_position=True,
                trade_peak_close=peak,
                trade_entry_idx=int(open_position["entry_idx"]),
            )
        ...
        if open_position is None:
            bars_since_exit = (index - last_exit_idx) if last_exit_idx is not None else None
            signal_eval = evaluate_signal(rows, index, in_position=False, bars_since_last_exit=bars_since_exit)
            sig = signal_eval.get("signal")
            if sig == SIDE_LONG:
                ...
                if sizing["can_enter"]:
                    ...
                    entry_role = fee_result["liquidity_role"]
                    net_size = size_usd - entry_fee

                    score = score_setup(signal_eval)

                    btc_units_local = net_size / close
                    cash -= size_usd

                    fees_paid += entry_fee
                    btc_units = btc_units_local
                    open_position = {
        ...
        mark_value = cash + (btc_units * close)
        equity_curve.append({"timestamp": str(row["date"]), "equity": round(mark_value, 2)})

    if open_position is not None:
        latest = rows[-1]
        signal_eval = evaluate_signal(rows, len(rows) - 1, in_position=True,
                                       trade_peak_close=float(open_position["peak_close"]),
                                       trade_entry_idx=int(open_position["entry_idx"]))
    ...
    diagnostics = build_trade_diagnostics(
        ...
        robust_gate=ASYMMETRIC_VOL_ROBUST_GATE,
    )
    ...
    return {
        ...
        "notes": [
            "BTC daily asymmetric vol carry replay using shared BTC/USD history.",
            "Long-only vol-regime: panic buying + compression breakout.",
            "No leverage; partial exposure per setup type.",
            "Validation requires beating the current 115.78% BTC convex cycle champion before paper review.",
        ],
    }
...
def close_position(
    ...
    return {
        ...
        "exit_cash_added": round(exit_cash_added, 8),
...
def build_latest_signal(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        **signal_eval,
        ...
        "invalidation_plan": (
            "Long exit: RSI<40 trend failure or 60-day time stop."
        ),
    }
