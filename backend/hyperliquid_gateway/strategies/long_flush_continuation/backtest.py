...
try:
    ...
    from backtesting.filters import build_snapshot_filter
    from backtesting.metrics import build_summary
from .logic import STRATEGY_ID, calculate_funding_percentile, evaluate_signal
...
def run_backtest(dataset_path: Path, config: BacktestConfig) -> dict[str, Any]:
    ...
    for row in sampled_rows:
        ...
        if len(history) < 12 or symbol in open_positions or closed_this_tick:
            continue
        ...
        entry_price = apply_slippage(float(market_data["price"]), "short", int(setup_score["execution_quality"]), is_exit=False)
        ...
        open_positions[symbol] = {
            ...
            "stop_loss": round(entry_price * 1.008, 6),
            "take_profit": round(entry_price * 0.986, 6),
            "entry_context": {
                "signal": signal_eval,
                "score": setup_score,
                "risk_plan": risk_plan,
    ...
    for position in list(open_positions.values()):
        ...
        trade = close_position(
            ...
            apply_slippage(float(latest_market_data["price"]), "short", int(latest_market_data.get("executionQuality", 60)), True),
            "forced_close",
            config,
        )
    ...
    return {
        ...
        "notes": [
            "Backend replay for long flush continuation using gateway market snapshots.",
            "Uses longs-at-risk crowding, positive/high funding, negative impulse, OI stability, and longFlush/fade setup scores.",
            "Passing validation only enables paper review; this is not a live-trading route.",
        ],
    }
...
def maybe_close_position(position: dict[str, Any], market_data: dict[str, Any], config: BacktestConfig) -> dict[str, Any] | None:
    ...
    execution_quality = int(market_data.get("executionQuality", 60))
    if price >= float(position["stop_loss"]):
        return close_position(position, market_data["timestamp"], apply_slippage(price, "short", execution_quality, True), "stop_loss", config)
    if price <= float(position["take_profit"]):
        return close_position(position, market_data["timestamp"], apply_slippage(price, "short", execution_quality, True), "take_profit", config)
    ...
    if oi_delta < -6.0:
        return close_position(position, market_data["timestamp"], apply_slippage(price, "short", execution_quality, True), "oi_collapse", config)
    ...
    pressure_persists = market_data.get("crowdingBias") == "longs-at-risk" or long_flush_score >= 55.0 or fade_score >= 60.0
    if not pressure_persists and held_ms >= ONE_HOUR_MS:
        return close_position(position, market_data["timestamp"], apply_slippage(price, "short", execution_quality, True), "pressure_fade", config)
    if held_ms >= TWO_HOURS_MS:
        return close_position(position, market_data["timestamp"], apply_slippage(price, "short", execution_quality, True), "time_stop", config)
    return None
...
def build_latest_signal(symbol_histories: dict[str, list[dict[str, Any]]]) -> dict[str, Any]:
    ...
    return {
        **signal_eval,
        ...
        "trigger_plan": "Enter short only after long-pressure scores or longs-at-risk crowding, positive/high funding, negative impulse, and OI stability align.",
        "invalidation_plan": "Exit on +0.8% stop, OI collapse, pressure fade, or 120 minute time stop.",
    }
...
def apply_slippage(price: float, side: str, execution_quality: int, is_exit: bool) -> float:
    ...
    if side == "short":
        return price * (1 + slippage) if is_exit else price * (1 - slippage)
    return price * (1 - slippage) if is_exit else price * (1 + slippage)
