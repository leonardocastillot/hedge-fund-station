...
def run_backtest(dataset_path: Path, config: BacktestConfig) -> dict[str, Any]:
    ...
    if not rows:
        raise ValueError("Structural Market Alpha requires non-empty BTC/USD daily history.")
    ...
    return {
        ...
        "notes": [
            "Structural market alpha using four independent factors: volatility cycle, volume structure, momentum quality, and market phase.",
            "Volatility cycle: score from ATR percentile (low vol = expand, high vol = contract).",
            "Volume structure: expands on low volume (quiet accumulation), contracts on high volume.",
            "Momentum quality: aligned multi-timeframe ROC + acceleration.",
            "Market phase: buy drawdowns, buy near ATH, buy recovery from deep drawdown.",
            "Composite scoring: dominant factor weighted by agreement bonus (no hardcoded winners).",
            "Entry: composite >= 45, trend bull, RSI 40-72, vol not extreme, pullback or momentum trigger.",
            "Exit: vol-scaled ATR trailing (2.5-5.5x continuous), trend break, time stop 200 days.",
            "Smooth sizing: risk_pct and exposure vary continuously with vol percentile + composite score.",
        ],
    }
...
def build_latest_signal(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        **signal_eval,
        ...
        "trigger_plan": "Enter long when composite >= 45, trend bull (close > SMA200, SMA50 > SMA200), "
                        "RSI 40-72, ATR percentile < 83, and pullback or momentum trigger.",
        "invalidation_plan": "Exit on vol-scaled ATR trailing stop (2.5-5.5x continuous), "
                            "trend structure break (close < SMA200 or SMA50 < SMA200), or time stop (200 days).",
    }
