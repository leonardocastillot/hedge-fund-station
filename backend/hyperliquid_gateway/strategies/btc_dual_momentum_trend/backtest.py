"""Backtest adapter for BTC Dual Momentum Trend."""
...
CHAMPION_STRATEGY = "btc_vol_atr_trend"
CHAMPION_RETURN_PCT = 162.30
DUAL_MOMENTUM_ROBUST_GATE = {
    "min_trades": 8,
    "min_return_pct": CHAMPION_RETURN_PCT,
    "min_profit_factor": 2.0,
    "max_drawdown_pct": 18.0,
    "min_avg_net_trade_return_pct": 2.0,
    "max_largest_trade_pnl_share_pct": 65.0,
}
...
def run_backtest(dataset_path: Path, config: BacktestConfig) -> dict[str, Any]:
    ...
    if not rows:
        raise ValueError("BTC Dual Momentum Trend requires non-empty BTC/USD daily history.")
    ...
    diagnostics = build_trade_diagnostics(
        ...
        robust_gate=DUAL_MOMENTUM_ROBUST_GATE,
    )
    ...
    return {
        ...
        "notes": [
            "BTC daily dual momentum trend replay using shared BTC/USD history.",
            "Long-only with ROC acceleration entry (ROC20 > ROC90) and momentum divergence exit.",
            "Position sizing via risk-budgeting (target_risk / ATR_stop) with 1.3x momentum bonus.",
            "Validation requires beating current 162.30% BTC vol ATR champion before paper review.",
        ],
    }
...
def build_latest_signal(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        **signal_eval,
        ...
        "trigger_plan": "Enter long when close > SMA200, SMA50 > SMA200, RSI14 > 40, ATR percentile < 85, "
                        "AND ROC20 > ROC90 (momentum accelerating). Size via risk-budgeting with 1.3x bonus under strong acceleration.",
        "invalidation_plan": "Exit on momentum divergence (ROC20<0 && ROC20<ROC90), "
                             "ATR trailing stop (4.0x ATR from peak), trend break (close < SMA200), or time stop (200 days).",
    }
